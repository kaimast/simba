...
pub(super) async fn create_material(renderer: &Renderer) -> Material {
    ...
    let program = renderer.get_program("rectangle").await;
...
fn compute_bounding_box(position: &Vec2, style: &RectangleStyle) -> BoundingBox {
    let offset = Vec2::new(
        style.width * 0.5 + style.border_width,
        style.height * 0.5 + style.border_width,
    );
...
pub(super) async fn new_drawable(
    ...
    let material = renderer.get_material("rectangle").await;
